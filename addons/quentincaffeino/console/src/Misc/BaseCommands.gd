...
func _init(console):
	...
	self._console.add_command('quit', self, '_quit')\
		.set_description('Exit the game.')\
		.register()

	self._console.add_command('exit', self, '_quit')\
		.set_description('Exit the game.')\
		.register()
	...
	self._console.add_command('fps_max', Engine, 'set_target_fps')\
		.set_description('The maximum framerate at which the game can run.')\
		.add_argument('fps', self._console.IntRangeType.new(10, 240))\
		.register()
...
func _help(command_name = null):
	if command_name:
		...
		self._console.write_line(\
			"Type [color=#ffff66][url=help]help[/url] <command-name>[/color] show information about a command.\n" + \
			"Type [color=#ffff66][url=commands]commands[/url][/color] to get a list of all available commands.\n" + \
			"Type [color=#ffff66][url=quit]quit[/url][/color] to exit the game.")
...
# @returns  void
func _quit():
	self._console.Log.warn('Quitting game...')
