...
var tocrouch_speed = 20
var default_height = 1.5
var crouch_height = 0.25
...
onready var glockLight = $Head/Hand/Glock/Muzzlelight
onready var glockFlash = $Head/Hand/Glock/MuzzleFlash2
...
func fireGlock():
	if Input.is_action_just_pressed("fire") and glock_can_shoot and not Console.is_console_shown:
		glock_can_shoot = false
		glockLight.show()
		glockFlash.show()
		glockTimer.start()
		player_anim.play("glockFire")
		$Audio/sndFire.pitch_scale = rand_range(0.95, 1.05)
		$Audio/sndFire.play()
...
func _physics_process(delta):
	...
	# - Head bobbing (currently overrides firing animation)
	#if direction != Vector3():
	#	player_anim.play("headBob")
